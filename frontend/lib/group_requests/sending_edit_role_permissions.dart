...
Future<List<String>> send_edited_permissions_for_role(
    String role_name, List<bool> edited_perms, String group_id) async {
  String temp_mssg = "";
  bool success_var = false;
  List<int> perms_data = [];
  for (int i = 0; i < edited_perms.length; i++) {
    if (edited_perms[i] == true) {
      perms_data.add(i + 1);
    }
  }
  Map data_to_be_sent = {

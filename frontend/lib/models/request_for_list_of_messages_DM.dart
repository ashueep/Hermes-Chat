...
request_for_list_of_messages_for_DM(int k) async {
  ...
  updated_mssg_model temp_message =
      updated_mssg_model(sender_username: "", text_message: "", time_stamp: "");

  var body = JsonEncoder().convert(jwt_token_var);
  var returned_list_of_DM_messages = await http.post(
      Uri.parse('$global_link/api/dms/$dm_id/view'),
      headers: {"Content-Type": "application/json"},
      body: body);

  print(returned_list_of_DM_messages);
  var decoded_return_list_of_DMs =
      jsonDecode(returned_list_of_DM_messages.body);
  print(decoded_return_list_of_DMs);
  List<updated_mssg_model> temp_update = [
    updated_mssg_model(sender_username: "", text_message: "", time_stamp: "")
  ];
  if (returned_list_of_DM_messages.statusCode == 201 ||
      returned_list_of_DM_messages.statusCode == 200) {
    ...
    for (int i = 0; i < size; i++) {
      updated_mssg_model temp_message1 = updated_mssg_model(
          sender_username: "", text_message: "", time_stamp: "");
    ...
    List<updated_mssg_model> reversed_list =
        new List.from(temp_update.reversed);
